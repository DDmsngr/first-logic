...
- update_task — изменить статус или срок СУЩЕСТВУЮЩЕЙ задачи, только если в сообщении назван её номер (#12 готово, #5 в работу, «задача 7 перенеси на пятницу»). Нет номера — это НЕ update_task.
- Фразы «сделай/создай/поставь/заведи задачу …» — всегда create_task; «ответственный/исполнитель я», «срок до завтра» — это поля этой же новой задачи (assignee_user_id, due_date), а не отдельная команда.
