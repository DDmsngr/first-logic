...
export interface UploadTarget { taskId?: string; messageId?: string; componentId?: string; supplierId?: string; productId?: string; assemblyId?: string }
...
export async function uploadFile(
  ...
  const ins = await supabase.from('ws_attachments').insert({
    ...
    component_id: target.componentId ?? null, supplier_id: target.supplierId ?? null, product_id: target.productId ?? null, assembly_id: target.assemblyId ?? null,
...
export async function fetchAttachments(opts: {
  taskId?: string; componentId?: string; supplierId?: string; productId?: string; assemblyId?: string; workspaceId?: string; messageIds?: string[]; page?: number; q?: string
}) {
...
export interface SearchResult {
  ...
  files: { id: string; task_id: string | null; component_id: string | null; supplier_id: string | null; product_id: string | null; assembly_id: string | null; filename: string; size: number }[]
  assemblies: { id: string; name: string; sku: string | null }[]
